...
val dataDF: DataFrame = spark.createDataFrame(dataRDD, StructType(Seq(
...
// New x values
val newValues = Seq(-1.5, 2.5, 8.5, 31.4)

// Define the schema for the new DataFrame
val newSchema = StructType(Seq(StructField("X", DoubleType, nullable = false)))

// Create a DataFrame with the new x values
val newRows = newValues.map(x => Row(x))
val newDF = spark.createDataFrame(spark.sparkContext.parallelize(newRows), newSchema)

// Apply the VectorAssembler to the new data
val assembledNewDF = vectorAssembler.transform(newDF)

// Use the trained LinearRegressionModel to predict y values for the new x values
val predictionsForNewData = lrModel.transform(assembledNewDF)

// Show the predictions for the new data
predictionsForNewData.select("X", "prediction").show()
...
val articlesRdd: RDD[String] = spark.sparkContext.textFile("abfss://shared@tunics320f2023gen2.dfs.core.windows.net/exercises/ex4")
...
val numberOfWords: Long = articlesRdd.flatMap(line => line.split(" ")).filter(_.nonEmpty).count()
println(s"numberOfWords: ${numberOfWords}")

val numberOfCharacters: Long = articlesRdd.map(line => line.replaceAll(" ", "").length.toLong).reduce(_ + _)
...
val words6Count: Long = articlesRdd
...
val mostCommon6LetterWord: (String, Long) = articlesRdd
  .flatMap(line => line.split("\\s+"))
  .filter(word => word.length == 6)
  .map(word => (word, 1L))
  .reduceByKey(_ + _)
...
val commonWord: String = mostCommon6LetterWord._1
val commonWordCount: Int = mostCommon6LetterWord._2.toInt
